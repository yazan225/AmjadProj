<?php
// Sales by Item Summary Report - Fixed Formatting with Tax Column
...
<head>
    ...
    <style>
        ...
        .main-table th:nth-child(1) { width: 7%; }   /* كود الصنف */
        .main-table th:nth-child(2) { width: 12%; }  /* اسم الصنف */
        .main-table th:nth-child(3) { width: 5%; }   /* الكمية */
        .main-table th:nth-child(4) { width: 6%; }   /* متوسط السعر */
        .main-table th:nth-child(5) { width: 6%; }   /* متوسط التكلفة */
        .main-table th:nth-child(6) { width: 7%; }   /* إجمالي المبيعات */
        .main-table th:nth-child(7) { width: 6%; }   /* نسبة الخصم % */
        .main-table th:nth-child(8) { width: 7%; }   /* قيمة الخصم */
        .main-table th:nth-child(9) { width: 7%; }   /* صافي المبيعات */
        .main-table th:nth-child(10) { width: 7%; }  /* الضريبة */
        .main-table th:nth-child(11) { width: 8%; }  /* مجمل البيع */
        .main-table th:nth-child(12) { width: 7%; }  /* قيمة التكلفة */
        .main-table th:nth-child(13) { width: 7%; }  /* صافي الربح */
        .main-table th:nth-child(14) { width: 6%; }  /* نسبة الربح % */
    </style>
</head>
<body>
    <div class="container">
        ...
        if (isset($_POST['Refresh'])) {
            ...
            // بناء الاستعلام الرئيسي من debtor_trans مباشرة
            $base_where = "dt.tran_date BETWEEN '$from_date' AND '$to_date' 
                          AND stk.inactive = 0 
                          AND dtd.quantity > 0";
            
            // إضافة فلتر نوع الحركات
            if ($sales_type == '10') {
                $base_where .= " AND dt.type = 10";
                $report_type_note = "عرض حركات المبيعات فقط (النوع 10)";
            } elseif ($sales_type == 'all_positive') {
                $base_where .= " AND dtd.quantity > 0";
            ...
            if (!empty($customer_id)) {
                $base_where .= " AND dt.debtor_no = '$customer_id'";
            }
            
            // إضافة فلتر الصنف
            if (!empty($item_code)) {
                $base_where .= " AND dtd.stock_id = '$item_code'";
                $report_type_note .= " | الصنف: " . $item_code;
            }

            // الاستعلام الرئيسي مع حساب الضريبة بشكل صحيح
            $sql = "SELECT 
                        dtd.stock_id,
                        stk.description,
                        SUM(dtd.quantity) as qty_sold,
                        SUM(dtd.quantity * dtd.unit_price) as gross_sales_amount,
                        SUM(dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0)) as total_discount_amount,
                        SUM(dtd.quantity * COALESCE(sm.standard_cost, stk.material_cost)) as cost_amount,
                        SUM(dtd.quantity * dtd.unit_tax) as total_tax_amount,
                        COUNT(DISTINCT CASE WHEN dt.tax_included = 1 THEN dt.trans_no END) as tax_included_count,
                        COUNT(DISTINCT CASE WHEN dt.tax_included = 0 THEN dt.trans_no END) as tax_excluded_count,
                        AVG(dtd.unit_price) as avg_price,
                        AVG(COALESCE(sm.standard_cost, stk.material_cost)) as avg_cost,
                        CASE 
                            WHEN SUM(dtd.quantity * dtd.unit_price) > 0 
                            THEN (SUM(dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0)) / SUM(dtd.quantity * dtd.unit_price)) * 100 
                            ELSE 0 
                        END as discount_percentage
                    FROM " . $prefix . "debtor_trans_details dtd
                    INNER JOIN " . $prefix . "debtor_trans dt ON dtd.debtor_trans_no = dt.trans_no AND dtd.debtor_trans_type = dt.type
                    INNER JOIN " . $prefix . "stock_master stk ON dtd.stock_id = stk.stock_id
                    LEFT JOIN " . $prefix . "stock_moves sm ON (dtd.stock_id = sm.stock_id AND dt.reference = sm.reference AND dt.tran_date = sm.tran_date)
                    WHERE $base_where
                    GROUP BY dtd.stock_id, stk.description
                    HAVING qty_sold > 0";

            // إضافة الترتيب
            switch ($order_by) {
                case 'stock_id':
                    $sql .= " ORDER BY dtd.stock_id";
                    break;
                case 'description':
                ...
                case 'qty_sold':
                    ...
                    break;
                case 'net_profit':
                    $sql .= " ORDER BY (SUM(dtd.quantity * dtd.unit_price) - SUM(dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0)) - SUM(dtd.quantity * COALESCE(sm.standard_cost, stk.material_cost))) DESC";
                    break;
                case 'sales_amount':
                default:
                    $sql .= " ORDER BY SUM(dtd.quantity * dtd.unit_price) DESC";
                    break;
            }
            ...
            if (!$result) {
                ...
                echo "<div class='alert alert-info'>استعلام التصحيح: " . $sql . "</div>";
            } else {
                ...
                if ($row_count > 0) {
                    ...
                    $total_cost = 0;
                    $total_net_profit = 0;
                    $total_tax_included_transactions = 0;
                    $total_tax_excluded_transactions = 0;
                    ...
                    if ($temp_result) {
                        while ($myrow = $temp_result->fetch_assoc()) {
                            ...
                            $total_tax += $myrow['total_tax_amount'];
                            $total_tax_included_transactions += $myrow['tax_included_count'];
                            $total_tax_excluded_transactions += $myrow['tax_excluded_count'];
                        }
                    }
                    
                    // حساب صافي المبيعات الصحيح حسب حالة الضريبة
                    // إذا كانت الضريبة ضمن السعر: صافي البيع = إجمالي البيع - الخصم - الضريبة
                    // إذا كانت الضريبة فوق السعر: صافي البيع = إجمالي البيع - الخصم
                    // لحساب ذلك بشكل صحيح، نحتاج إلى استعلام منفصل
                    $net_sales_sql = "SELECT 
                        SUM(CASE 
                            WHEN dt.tax_included = 1 THEN (dtd.quantity * dtd.unit_price) - (dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0)) - (dtd.quantity * dtd.unit_tax)
                            WHEN dt.tax_included = 0 THEN (dtd.quantity * dtd.unit_price) - (dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0))
                            ELSE (dtd.quantity * dtd.unit_price) - (dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0))
                        END) as correct_net_sales
                    FROM " . $prefix . "debtor_trans_details dtd
                    INNER JOIN " . $prefix . "debtor_trans dt ON dtd.debtor_trans_no = dt.trans_no AND dtd.debtor_trans_type = dt.type
                    INNER JOIN " . $prefix . "stock_master stk ON dtd.stock_id = stk.stock_id
                    WHERE $base_where";
                    
                    $net_sales_result = $conn->query($net_sales_sql);
                    if ($net_sales_result && $net_sales_result->num_rows > 0) {
                        $net_row = $net_sales_result->fetch_assoc();
                        $total_net_sales = $net_row['correct_net_sales'] ?: 0;
                    } else {
                        // حساب تقريبي إذا فشل الاستعلام
                        $total_net_sales = $total_gross_sales - $total_discount;
                    }
                    
                    // حساب مجمل البيع الصحيح
                    // مجمل البيع = صافي المبيعات + الضريبة
                    $total_gross_with_tax = $total_net_sales + $total_tax;
                    
                    $total_net_profit = $total_net_sales - $total_cost;
                    ...
                    echo "<div class='stat-item'>";
                    echo "<div class='stat-value' style='color:#9C27B0'>" . format_number($total_tax, 2) . " ₪</div>";
                    echo "<div class='stat-label'>إجمالي الضريبة</div>";
                    echo "</div>";
                    
                    echo "<div class='stat-item'>";
                    echo "<div class='stat-value' style='color:#2196F3'>" . format_number($total_gross_with_tax, 2) . " ₪</div>";
                    echo "<div class='stat-label'>مجمل البيع</div>";
                    echo "</div>";
                    ...
                    echo "<div style='text-align:center; margin-top:5px; color:#9C27B0; font-weight:bold;'>";
                    echo "شامل الضريبة: " . $total_tax_included_transactions . " فاتورة | خارج الضريبة: " . $total_tax_excluded_transactions . " فاتورة";
                    echo "</div>";
                    if ($show_details) {
                    ...
                    while ($myrow = $result->fetch_assoc()) {
                        ...
                        $tax_amount = $myrow['total_tax_amount'];
                        
                        // حساب صافي المبيعات الصحيح لكل صنف
                        $item_net_sales_sql = "SELECT 
                            SUM(CASE 
                                WHEN dt.tax_included = 1 THEN (dtd.quantity * dtd.unit_price) - (dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0)) - (dtd.quantity * dtd.unit_tax)
                                WHEN dt.tax_included = 0 THEN (dtd.quantity * dtd.unit_price) - (dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0))
                                ELSE (dtd.quantity * dtd.unit_price) - (dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0))
                        ...
                        FROM " . $prefix . "debtor_trans_details dtd
                        INNER JOIN " . $prefix . "debtor_trans dt ON dtd.debtor_trans_no = dt.trans_no AND dtd.debtor_trans_type = dt.type
                        WHERE dtd.stock_id = '" . $myrow['stock_id'] . "' 
                        AND dt.tran_date BETWEEN '$from_date' AND '$to_date'";
                        
                        $item_net_result = $conn->query($item_net_sales_sql);
                        if ($item_net_result && $item_net_result->num_rows > 0) {
                            $item_net_row = $item_net_result->fetch_assoc();
                            $net_sales = $item_net_row['correct_net_sales'] ?: 0;
                        } else {
                            $net_sales = $gross_sales - $discount_amount;
                        }
                        
                        // حساب مجمل البيع الصحيح
                        $gross_with_tax = $net_sales + $tax_amount;
                        ...
                        $tax_amount_fmt = format_number($tax_amount, 2);
                        $gross_with_tax_fmt = format_number($gross_with_tax, 2);
                        ...
                        echo "<td style='text-align:center; color:#9C27B0; font-weight:bold;'>{$tax_amount_fmt} ₪</td>";
                        echo "<td style='text-align:center; color:#2196F3; font-weight:bold;'>{$gross_with_tax_fmt} ₪</td>";
                        ...
                        if ($show_details) {
                            // استعلام التفاصيل مع حالة الضريبة
                            $detail_sql = "SELECT 
                                        dt.trans_no,
                                        dt.reference,
                                        dt.tran_date,
                                        ...
                                        dt.tax_included,
                                        dtd.quantity as qty, 
                                        dtd.unit_price as price, 
                                        dtd.unit_tax as unit_tax,
                                        COALESCE(sm.standard_cost, stk.material_cost) as standard_cost,
                                        (dtd.quantity * dtd.unit_price) as gross_line_total,
                                        (dtd.quantity * COALESCE(sm.standard_cost, stk.material_cost)) as line_cost,
                                        (dtd.quantity * dtd.unit_tax) as line_tax,
                                        COALESCE(dtd.discount_percent, 0) as discount_rate,
                                        (dtd.quantity * dtd.unit_price * COALESCE(dtd.discount_percent, 0)) as line_discount,
                                        dt.debtor_no,
                            ...
                            // Sort by transaction number and date descending
                            $detail_sql .= " ORDER BY dt.tran_date DESC, dt.trans_no DESC";
                            
                            $detail_result = $conn->query($detail_sql);
                            
                            if ($detail_result && $detail_result->num_rows > 0) {
                                // استخدام div منظم لعرض التفاصيل
                                echo "<tr><td colspan='14' style='padding:0;'>";
                                ...
                                echo "<tr>";
                                echo "<th style='width:6%;'>التاريخ</th>";
                                echo "<th style='width:5%;'>رقم الفاتورة</th>";
                                echo "<th style='width:6%;'>المرجع</th>";
                                echo "<th style='width:4%;'>النوع</th>";
                                echo "<th style='width:4%;'>ضريبة</th>";
                                echo "<th style='width:4%;'>الكمية</th>";
                                echo "<th style='width:5%;'>سعر الوحدة</th>";
                                echo "<th style='width:6%;'>إجمالي السعر</th>";
                                echo "<th style='width:5%;'>نسبة الخصم</th>";
                                echo "<th style='width:6%;'>قيمة الخصم</th>";
                                echo "<th style='width:6%;'>صافي السعر</th>";
                                echo "<th style='width:5%;'>ضريبة الوحدة</th>";
                                echo "<th style='width:6%;'>إجمالي الضريبة</th>";
                                echo "<th style='width:6%;'>مجمل البيع</th>";
                                echo "<th style='width:6%;'>تكلفة الوحدة</th>";
                                echo "<th style='width:6%;'>إجمالي التكلفة</th>";
                                echo "<th style='width:7%;'>الربح</th>";
                                echo "<th style='width:8%;'>العميل</th>";
                                echo "</tr>";
                                ...
                                $detail_total_tax = 0;
                                $detail_total_gross_with_tax = 0;
                                $detail_total_cost = 0;
                                $detail_total_profit = 0;
                                $detail_tax_included_count = 0;
                                $detail_tax_excluded_count = 0;
                                ...
                                while ($detail = $detail_result->fetch_assoc()) {
                                    ...
                                    if ($detail['tax_included'] == 1) {
                                        // الضريبة ضمن السعر: صافي البيع = السعر بعد الخصم - الضريبة
                                        $line_net_sales = $line_net_sales - $detail['line_tax'];
                                        $detail_tax_included_count++;
                                        $tax_status = "شامل";
                                        $tax_status_color = "#2196F3";
                                    } else {
                                        // الضريبة فوق السعر: صافي البيع = السعر بعد الخصم
                                        $detail_tax_excluded_count++;
                                        $tax_status = "خارج";
                                        $tax_status_color = "#FF9800";
                                    }
                                    
                                    $line_tax = $detail['line_tax'];
                                    $line_gross_with_tax = $line_net_sales + $line_tax;
                                    $line_profit = $line_net_sales - $detail['line_cost'];
                                    
                                    echo "<tr $detail_bg>";
                                    echo "<td>" . $detail['tran_date'] . "</td>";
                                    echo "<td style='font-weight:bold;'>" . $detail['trans_no'] . "</td>";
                                    echo "<td>" . $detail['reference'] . "</td>";
                                    echo "<td>" . $detail['type'] . "</td>";
                                    echo "<td style='color:$tax_status_color; font-weight:bold;'>" . $tax_status . "</td>";
                                    echo "<td>" . format_number($detail['qty'], 0) . "</td>";
                                    echo "<td>" . format_number($detail['price'], 2) . " ₪</div>";
                                    ...
                                    echo "<td style='color:#9C27B0;'>" . format_number($detail['unit_tax'], 2) . " ₪</td>";
                                    echo "<td style='color:#9C27B0;'>" . format_number($line_tax, 2) . " ₪</td>";
                                    echo "<td style='color:#2196F3; font-weight:bold;'>" . format_number($line_gross_with_tax, 2) . " ₪</td>";
                                    ...
                                    $detail_total_tax += $line_tax;
                                    $detail_total_gross_with_tax += $line_gross_with_tax;
                                ...
                                echo "<td colspan='5'>إجمالي الحركات</td>";
                                ...
                                echo "<td>-</td>";
                                echo "<td style='color:#9C27B0;'>" . format_number($detail_total_tax, 2) . " ₪</td>";
                                echo "<td style='color:#2196F3;'>" . format_number($detail_total_gross_with_tax, 2) . " ₪</td>";
                                echo "<td>-</td>";
                                ...
                                echo "<td colspan='2'>شامل ضريبة: {$detail_tax_included_count} | خارج ضريبة: {$detail_tax_excluded_count}</td>";
                                echo "</tr>";
                    ...
                    echo "<td style='text-align:center; color:#9C27B0;'>" . format_number($total_tax, 2) . " ₪</td>";
                    echo "<td style='text-align:center; color:#2196F3;'>" . format_number($total_gross_with_tax, 2) . " ₪</td>";
                    ...
                    echo "فاتورة شامل الضريبة: " . $total_tax_included_transactions . " | ";
                    echo "فاتورة خارج الضريبة: " . $total_tax_excluded_transactions;
                    if ($show_details) {
                        echo " | مع التفاصيل";
                    }
                    echo "</div>";
            ...
            echo "<p style='color:#9C27B0; font-weight:bold;'>ملاحظة: يتم حساب الضريبة من حقل unit_tax في debtor_trans_details</p>";
            echo "<p style='color:#2196F3; font-weight:bold;'>شامل الضريبة: صافي البيع = السعر بعد الخصم - الضريبة</p>";
            echo "<p style='color:#FF9800; font-weight:bold;'>خارج الضريبة: صافي البيع = السعر بعد الخصم</p>";
            echo "</div>";
        }
