...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    try {
        ...
        $order_no = ($row_order['max_order'] ?? 0) + 1;
        ...
        // التصحيح: التأكد من أن from_stk_loc هو القيمة الصحيحة للموقع
        ...
        error_log("from_stk_loc: " . $default_location); // هذا هو المهم
        ...
        $stmt_order->bind_param("isssssissd", 
            ...
            $tran_date,
            $ship_via,
            $default_location,  // هذا هو حقل from_stk_loc
            $due_date,
            $payment_term,
            $total
        );
        ...
        if (!$stmt_order->execute()) {
            throw new Exception("فشل في إنشاء طلب المبيعات: " . $stmt_order->error . " - القيم: order_no=$order_no, debtor_no=$walkin_debtor_no, branch_code=$branch_code, from_stk_loc=$default_location");
        }
        ...
        // حفظ الرسالة في الجلسة لضمان ظهورها بعد إعادة التوجيه
        $_SESSION['success_message'] = "تم حفظ الفاتورة بنجاح! رقم: $trans_no (طلب المبيعات: $order_no) - العملة: $main_currency - الموقع: $default_location";
...
// التعامل مع معالجة الدفع - الكود المصحح
        ...
        if ($paid_amount <= 0) {
            ...
            if ($change < 0) {
                ...
                try {
                    ...
                    // التحقق من وجود حقل curr_code في جدول debtor_trans للدفعات
                    ...
                    // التصحيح: استخدام + في تحديث حقل alloc
