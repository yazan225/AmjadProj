...
// الحصول على جميع مراكز التكلفة - التصحيح النهائي
$cost_centers = [];
...
if ($result_cost_centers) {
    if ($result_cost_centers->num_rows > 0) {
        ...
        error_log("لا توجد مراكز تكلفة في الجدول {$prefix}dimensions");
    }
} else {
    error_log("خطأ في استعلام مراكز التكلفة: " . $conn->error);
    // جرب جدول بديل إذا كان موجوداً
    $sql_cost_centers_alt = "SHOW TABLES LIKE '{$prefix}dimensions%'";
    $result_alt = $conn->query($sql_cost_centers_alt);
    $alt_tables = [];
    while ($row = $result_alt->fetch_array()) {
        $alt_tables[] = $row[0];
    }
    error_log("الجداول البديلة المتاحة: " . implode(', ', $alt_tables));
}
...
if (!isset($_SESSION['selected_cost_center'])) {
    if (count($cost_centers) > 0) {
        ...
        error_log("تم تعيين مركز التكلفة الافتراضي إلى 0 لعدم وجود مراكز");
    }
}

// التعامل مع تغيير مركز التكلفة
...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    try {
        ...
        error_log("بدء حفظ الفاتورة - الزبون: " . $walkin_debtor_no . ", الفرع: " . $branch_code . ", العملة: " . $main_currency . ", الموقع: " . $default_location . ", مركز التكلفة: " . $default_cost_center);
        ...
        // التحقق من وجود أعمدة dimension في جدول sales_orders
        $sql_check_dimension = "SHOW COLUMNS FROM {$prefix}sales_orders LIKE 'dimension_id'";
        $result_check_dimension = $conn->query($sql_check_dimension);
        $has_dimension = $result_check_dimension && $result_check_dimension->num_rows > 0;
        ...
        if ($has_dimension) {
            $sql_order = "INSERT INTO {$prefix}sales_orders 
                ...
                delivery_date, payment_terms, total, prep_amount, alloc, dimension_id, dimension2_id) 
                VALUES (?, 30, 0, 1, ?, ?, ?, '', '', ?, 1, ?, '', '', '', '', 0, ?, ?, ?, ?, 0, 0, ?, 0)";
            ...
            error_log("dimension_id: " . $default_cost_center);

            $stmt_order->bind_param("issssssssdii", 
                $order_no,
                $walkin_debtor_no,
                $branch_code,
                $order_ref,
                $tran_date,
                $ship_via,
                $default_location,  // ✅ التصحيح: استخدام القيمة الصحيحة للموقع المحدد
                $due_date,
                $payment_term,
                $total,
                $default_cost_center
        ...
        // التحقق من وجود أعمدة dimension في جدول debtor_trans
        $sql_check_dimension_dt = "SHOW COLUMNS FROM {$prefix}debtor_trans LIKE 'dimension_id'";
        $result_check_dimension_dt = $conn->query($sql_check_dimension_dt);
        $has_dimension_dt = $result_check_dimension_dt && $result_check_dimension_dt->num_rows > 0;

        if ($has_dimension_dt) {
            // إدخال حركة المدين مع dimension_id
            if ($has_curr_code) {
                $sql = "INSERT INTO {$prefix}debtor_trans
                    (trans_no, type, version, debtor_no, branch_code, tran_date, due_date, 
                    reference, tpe, ship_via, payment_terms, tax_included, ov_amount, ov_discount, order_, curr_code, dimension_id, dimension2_id)
                    VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)";
                ...
                $stmt->bind_param("iiissssiisiddisii", 
                    $trans_no, 
                    $type, 
                    $walkin_debtor_no, 
                    $branch_code, 
                ...
                $sql = "INSERT INTO {$prefix}debtor_trans
                    (trans_no, type, version, debtor_no, branch_code, tran_date, due_date, 
                    reference, tpe, ship_via, payment_terms, tax_included, ov_amount, ov_discount, order_, dimension_id, dimension2_id)
                    VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)";
                ...
                $stmt->bind_param("iiissssiisiddiii", 
                    $trans_no, 
                    $type, 
                    $walkin_debtor_no, 
                    $branch_code, 
        ...
        foreach ($_SESSION['cart'] as $item) {
            ...
            // التحقق من وجود أعمدة dimension في جدول stock_moves
            $sql_check_dimension_stock = "SHOW COLUMNS FROM {$prefix}stock_moves LIKE 'dimension_id'";
            $result_check_dimension_stock = $conn->query($sql_check_dimension_stock);
            $has_dimension_stock = $result_check_dimension_stock && $result_check_dimension_stock->num_rows > 0;
            
            // إضافة حركة المخزون - التأكد من استخدام الموقع الصحيح
            if ($has_curr_code_stock && $has_dimension_stock) {
                $sql_stock_move = "INSERT INTO {$prefix}stock_moves
                    (trans_no, type, stock_id, loc_code, tran_date, price, reference, qty, standard_cost, curr_code, dimension_id, dimension2_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)";
                ...
                $stmt_stock_move->bind_param("iisssdsddsi", 
                    ...
                    $type, 
                    $item['stock_id'], 
                    $default_location,  // ✅ استخدام الموقع المحدد
                    $tran_date, 
                    $item['unit_price'], 
                    $reference, 
                    $qty, 
            ...
            } elseif ($has_dimension_stock) {
                $sql_stock_move = "INSERT INTO {$prefix}stock_moves
                    (trans_no, type, stock_id, loc_code, tran_date, price, reference, qty, standard_cost, dimension_id, dimension2_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)";
                $stmt_stock_move = $conn->prepare($sql_stock_move);
                $qty = -abs($item['qty']);
        ...
        $memo_ = "فاتورة مبيعات رقم " . $trans_no . " - العملة: " . $main_currency . " - الموقع: " . $default_location . " - مركز التكلفة: " . $default_cost_center;
        ...
        // إتمام المعاملة
        $conn->commit();
        $success = "تم حفظ الفاتورة بنجاح! رقم: $trans_no (طلب المبيعات: $order_no) - العملة: $main_currency - الموقع: $default_location - مركز التكلفة: $default_cost_center";
...
// التعامل مع معالجة الدفع - الإصدار المصحح
        ...
        if ($paid_amount <= 0) {
            ...
            if ($change < 0) {
                ...
                try {
                   ...
                   // إضافة القيود المحاسبية - التصحيح النهائي
$sales_account = "41010001";
$receivable_account = "11011001"; 
$inventory_account = "1400";
$cogs_account = "5000";

// التحقق من وجود جدول company
$sql_check_company = "SHOW TABLES LIKE '{$prefix}company'";
$result_check_company = $conn->query($sql_check_company);

if ($result_check_company && $result_check_company->num_rows > 0) {
    $sql_accounts = "SELECT sales_account, receivable_account, inventory_account, cogs_account 
                     FROM {$prefix}company LIMIT 1";
    $result_accounts = $conn->query($sql_accounts);
    if ($result_accounts && $result_accounts->num_rows > 0) {
        $company_accounts = $result_accounts->fetch_assoc();
        if (!empty($company_accounts['sales_account'])) $sales_account = $company_accounts['sales_account'];
        if (!empty($company_accounts['receivable_account'])) $receivable_account = $company_accounts['receivable_account'];
        if (!empty($company_accounts['inventory_account'])) $inventory_account = $company_accounts['inventory_account'];
        if (!empty($company_accounts['cogs_account'])) $cogs_account = $company_accounts['cogs_account'];
    }
} else {
    error_log("⚠️ جدول company غير موجود، استخدام الحسابات الافتراضية");
}

                    // الحصول على حساب النقدية - التصحيح النهائي
                    $sql_check_bank = "SHOW TABLES LIKE '{$prefix}bank_accounts'";
                    $result_check_bank = $conn->query($sql_check_bank);

                    if ($result_check_bank && $result_check_bank->num_rows > 0) {
                        // التحقق من وجود الحقول في جدول bank_accounts
                        $sql_check_columns = "SHOW COLUMNS FROM {$prefix}bank_accounts";
                        $result_columns = $conn->query($sql_check_columns);
                        $has_bank_account = false;
                        $has_account_code = false;
                        
                        while ($col = $result_columns->fetch_assoc()) {
                            if ($col['Field'] == 'bank_account') $has_bank_account = true;
                            if ($col['Field'] == 'account_code') $has_account_code = true;
                        }
                        
                        if ($has_bank_account) {
                            $sql_cash_acc = "SELECT bank_account FROM {$prefix}bank_accounts WHERE inactive = 0 LIMIT 1";
                        } elseif ($has_account_code) {
                            $sql_cash_acc = "SELECT account_code as bank_account FROM {$prefix}bank_accounts WHERE inactive = 0 LIMIT 1";
                        } else {
                            $sql_cash_acc = "SELECT bank_account FROM {$prefix}bank_accounts WHERE inactive = 0 LIMIT 1";
                        }
                    ...
                    // التحقق من وجود أعمدة dimension في جدول debtor_trans للدفع
                    $sql_check_dimension_payment = "SHOW COLUMNS FROM {$prefix}debtor_trans LIKE 'dimension_id'";
                    $result_check_dimension_payment = $conn->query($sql_check_dimension_payment);
                    $has_dimension_payment = $result_check_dimension_payment && $result_check_dimension_payment->num_rows > 0;
                    
                    // إدخال حركة الدفع
                    if ($has_dimension_payment) {
                        $sql_payment = "INSERT INTO {$prefix}debtor_trans
                            (trans_no, type, version, debtor_no, branch_code, tran_date, due_date, reference, tpe, ov_amount, ov_discount, alloc, dimension_id, dimension2_id)
                            VALUES (?, ?, 0, ?, ?, ?, ?, ?, 0, ?, 0, ?, ?, 0)";
                        ...
                        // ربط جميع المعلمات المطلوبة
                        $stmt_payment->bind_param("iiissssddi", 
                            $payment_trans_no, 
                            $payment_type, 
                            $walkin_debtor_no, 
                            $branch_code,
                        ...
                        // ربط جميع المعلمات المطلوبة
                        $stmt_payment->bind_param("iiissssdd", 
                            $payment_trans_no, 
                            $payment_type, 
                            $walkin_debtor_no, 
                            $branch_code,
                    ...
                    $memo_ = "دفعة نقدية للفاتورة رقم " . $invoice_no;
...
<body>
    ...
    <select name="cost_center" id="cost_center">
        <?php if (count($cost_centers) > 0): ?>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <option value="0">⚠️ لا توجد مراكز تكلفة - تحقق من جدول dimensions</option>
        <?php endif; ?>
    </select>
    ...
    <script>
        ...
        function openPaymentModal() {
            ...
            if (cartItems.length > 0) {
                ...
                total = subTotal; // تم إزالة الخصم الكلي
                currentInvoiceNo = '';
