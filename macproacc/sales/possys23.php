...
function calculateInvoiceTotals($cart_items, $tax_rate, $tax_included_in_price) {
    ...
    if ($tax_rate > 0) {
        if ($tax_included_in_price == 1) {
            ...
            // المجموع الفرعي = السعر الإجمالي (شامل الضريبة)
            // لحساب الضريبة: نستخرجها من السعر الإجمالي
            ...
            // المجموع الفرعي = السعر بدون ضريبة
            $net_sales = $sub_total;
...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    try {
        ...
        // 4. إدخال في debtor_trans (الطريقة المرنة)
        // أولاً: التحقق من هيكل الجدول
        $sql_check = "DESCRIBE {$prefix}debtor_trans";
        $result_check = $conn->query($sql_check);
        $debtor_trans_fields = [];
        while ($field = $result_check->fetch_assoc()) {
      ...
      $basic_fields = [
    'trans_no' => ['value' => $trans_no, 'type' => 'i'],
    'type' => ['value' => $type, 'type' => 'i'],
    'version' => ['value' => 1, 'type' => 'i'],  // ← التعديل هنا (من 0 إلى 1)
    'debtor_no' => ['value' => $walkin_debtor_no, 'type' => 'i'],
    'branch_code' => ['value' => $branch_code, 'type' => 's'],
    'tran_date' => ['value' => $tran_date, 'type' => 's'],
    'due_date' => ['value' => $tran_date, 'type' => 's'],
    'reference' => ['value' => $reference, 'type' => 's'],
    'tpe' => ['value' => $default_sales_type, 'type' => 'i'],
    'ov_amount' => ['value' => $total_for_invoice, 'type' => 'd'],
    'tax_included' => ['value' => $tax_included_in_price, 'type' => 'i'],
    'dimension_id' => ['value' => $default_cost_center, 'type' => 'i'],
    'order_' => ['value' => 1, 'type' => 'i']  // ← التعديل هنا (من 0 إلى 1)
];
        
        foreach ($basic_fields as $field => $data) {
        ...
        $stmt_debtor = $conn->prepare($sql_debtor_trans);
        if (!$stmt_debtor) {
            throw new Exception("خطأ في تحضير استعلام debtor_trans: " . $conn->error);
        }
        
        // ربط القيم
        $stmt_debtor->bind_param($types, ...$values);
        
        if (!$stmt_debtor->execute()) {
            // إذا فشل، نستخدم طريقة أبسط
            $stmt_debtor->close();
            
            $sql_simple = "INSERT INTO {$prefix}debtor_trans 
                (trans_no, type, debtor_no, branch_code, tran_date, due_date, 
                reference, tpe, ov_amount, tax_included) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            $stmt_simple = $conn->prepare($sql_simple);
            $stmt_simple->bind_param("iiissssidii", 
                $trans_no, $type, $walkin_debtor_no, $branch_code, 
                $tran_date, $tran_date, $reference, $default_sales_type, 
                $total_for_invoice, $tax_included_in_price
            );
            
            if (!$stmt_simple->execute()) {
                throw new Exception("فشل في حفظ الفاتورة (debtor_trans): " . $stmt_simple->error . 
                    "<br>SQL: " . $sql_simple);
            }
            $stmt_simple->close();
        } else {
...
<head>
    ...
    <style>
        * {
        ...
        .invoice-details .panel-title {
            color: var(--secondary-dark);
            border-bottom-color: #c4e6d4;
        }
...
<body>
            ...
            <div class="panel right-panel">
                ...
                <form method="post" class="add-item-section">
                    <div class="input-group">
                        <input type="text" name="item_code" id="item_code_input" placeholder="أدخل الباركود/اسم الصنف" autofocus>
                ...
                <div class="customer-info">
                    ...
                    <div class="customer-row">
                        ...
                        <span><?= $selected_sales_type_name ?></span>
                    </div>
                    <div class="customer-row">
                        <span>نوع الضريبة:</span>
                        <span><?= $selected_tax_name ?></span>
                    </div>
                </div>
                ...
                <div class="invoice-details show">
                    <h3 class="panel-title"><i class="fas fa-file-invoice"></i> فاتورة محفوظة</h3>
                    <div class="invoice-row">
                        <span>رقم الفاتورة:</span>
                        <span><?= $_SESSION['last_invoice']['trans_no'] ?></span>
                    </div>
                    <div class="invoice-row">
                        <span>التاريخ:</span>
                        <span><?= $_SESSION['last_invoice']['date'] ?></span>
                    </div>
                    <div class="invoice-row">
                        <span>المبيعات الصافية:</span>
                        <span><?= number_format($_SESSION['last_invoice']['net_sales'], 2) ?> ₪</span>
                    </div>
                    <?php if ($_SESSION['last_invoice']['tax_amount'] > 0): ?>
                    <div class="invoice-row">
                        <span>الضريبة (<?= $_SESSION['last_invoice']['tax_rate'] ?>%):</span>
                        <span><?= number_format($_SESSION['last_invoice']['tax_amount'], 2) ?> ₪</span>
                    </div>
                    <div class="invoice-row">
                        <span>حالة الضريبة:</span>
                        <span><?= $_SESSION['last_invoice']['tax_included'] == 1 ? 'مضمنة في السعر' : 'مضافة على السعر' ?></span>
                    </div>
                    <?php endif; ?>
                    <div class="invoice-row invoice-total">
                        <span>المجموع الكلي:</span>
                        <span><?= number_format($_SESSION['last_invoice']['total'], 2) ?> ₪</span>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="quick-actions">
                    ...
                    <button class="btn btn-warning" onclick="printInvoice('a4')">
                        <i class="fas fa-print"></i> طباعة A4 (F10)
                    </button>
                    <button class="btn btn-primary" onclick="printInvoice('receipt')">
                        <i class="fas fa-receipt"></i> طباعة إيصال (F9)
                    </button>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // إظهار قسم الفاتورة المحفوظة إذا كان هناك فاتورة
            if (lastInvoice) {
