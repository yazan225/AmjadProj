...
// التعامل مع تغيير المخزن ومركز التكلفة
...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    try {
        ...
        // إضافة القيود المحاسبية مع مركز التكلفة
        ...
        $memo_ = "فاتورة مبيعات رقم " . $trans_no . " - العملة: " . $main_currency . " - الموقع: " . $default_location . " - مركز التكلفة: " . $dimension_id;
        ...
        // إتمام المعاملة
        $conn->commit();
        $success = "تم حفظ الفاتورة بنجاح! رقم: $trans_no (طلب المبيعات: $order_no) - العملة: $main_currency - الموقع: $default_location - مركز التكلفة: $dimension_id";
    ...
    if (!isset($_POST['paid_amount']) || !isset($_POST['invoice_total']) || !isset($_POST['invoice_no'])) {
        ...
        if ($paid_amount <= 0) {
            ...
            if ($change < 0) {
                ...
                // استخدام مركز التكلفة من الجلسة
                $dimension_id = $default_cost_center;
                
                // التحقق من وجود حقل dimension_id في جدول debtor_trans للدفعات
                $sql_check_dimension_payment = "SHOW COLUMNS FROM {$prefix}debtor_trans LIKE 'dimension_id'";
                $result_check_dim_payment = $conn->query($sql_check_dimension_payment);
                $has_dimension_payment = $result_check_dim_payment && $result_check_dim_payment->num_rows > 0;
                
                // إدخال حركة الدفع
                if ($has_dimension_payment) {
                    $sql_payment = "INSERT INTO {$prefix}debtor_trans
                        (trans_no, type, version, debtor_no, branch_code, tran_date, due_date, reference, tpe, ov_amount, ov_discount, alloc, dimension_id)
                        VALUES (?, ?, 0, ?, ?, ?, ?, ?, 0, ?, 0, ?, ?)";
                    $stmt_payment = $conn->prepare($sql_payment);
                    
                    if (!$stmt_payment) {
                        $error = "فشل في تحضير استعلام الدفع: " . $conn->error;
                    } else {
                        $stmt_payment->bind_param("iiissssdddi", 
                            $payment_trans_no, 
                            $payment_type, 
                            $walkin_debtor_no, 
                            $branch_code,
                ...
                if ($stmt_payment && !$stmt_payment->execute()) {
                    ...
                    if (!$stmt_update) {
                        $error = "فشل في تحضير استعلام التحديث: " . $conn->error;
                    } else {
                        ...
                        if (!$stmt_update->execute()) {
                            ...
                            // إضافة قيد محاسبي للدفعة مع مركز التكلفة
                            try {
                                $sql_gl = "INSERT INTO {$prefix}gl_trans 
                                          (type, type_no, tran_date, account, memo_, amount, dimension_id, dimension2_id, person_type_id, person_id)
                                          VALUES (?, ?, ?, ?, ?, ?, ?, 0, 2, ?)";
                                ...
                                $cash_account = "11010001"; // حساب النقدية الافتراضي
                                ...
                                $receivable_account = "11011001"; // حساب الذمم المدينة
                                ...
                                // لا نوقف العملية إذا فشل القيد المحاسبي، فقط نسجل الخطأ
                                error_log("خطأ في القيود المحاسبية للدفعة: " . $e->getMessage());
                            }
                        }
                    }
                }
