...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    // حساب الضريبة للإرسال
    $tax_amount_for_invoice = 0;
    if ($current_tax_rate > 0) {
        $tax_amount_for_invoice = $sub_total_for_invoice * $current_tax_rate;
    }
    
    $total_for_invoice = $sub_total_for_invoice + $tax_amount_for_invoice;
    ...
    try {
        ...
        $tax_amount_for_gl = $sub_total_for_invoice * $tax_rate_for_calc;
        $net_amount = $sub_total_for_invoice;
