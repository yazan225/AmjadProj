...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    try {
        ...
        $optional_fields = ['ov_gst', 'ov_freight', 'ov_freight_tax', 'prep_amount', 'dimension2_id'];
        foreach ($optional_fields as $field) {
            if (in_array($field, $debtor_trans_fields)) {
                $debtor_data[$field] = ($field == 'ov_gst') ? $tax_amount_for_invoice : 0;
            }
        }
        ...
        // 5. إدخال تفاصيل الفاتورة
        foreach ($_SESSION['cart'] as $item) {
            $discount_percent = $item['discount'] / 100;
            
            $sql_details = "INSERT INTO {$prefix}debtor_trans_details
                (debtor_trans_no, debtor_trans_type, stock_id, description, unit_price, 
                quantity, discount_percent, standard_cost, qty_done)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            $stmt_details->bind_param("iissddddd", 
                $trans_no, $type, $item['stock_id'], $item['description'],
                $item['unit_price'], $item['qty'], $discount_percent,
        ...
        // 7. إدخال تفاصيل الضريبة إذا وجدت
        if ($tax_amount_for_invoice > 0 && $selected_tax_data) {
            $sql_tax = "INSERT INTO {$prefix}trans_tax_details 
                (trans_type, trans_no, tran_date, tax_type_id, rate, amount)
                VALUES (?, ?, ?, ?, ?, ?)";
            
            $stmt_tax = $conn->prepare($sql_tax);
            if ($stmt_tax) {
                $stmt_tax->bind_param("iisiid", 
                    $type, $trans_no, $tran_date,
                    $default_tax_type, $selected_tax_data['rate'],
        ...
        $success = "✅ تم حفظ الفاتورة بنجاح!<br>" . 
...
<head>
    ...
    <style>
        * {
        ...
        /* تصميم قسم معلومات الزبون الجديد */
        .customer-card {
            margin-top: 20px;
...
<body>
    <div class="container">
        ...
        <div class="content">
            <div class="panel right-panel">
                ...
                <!-- قسم معلومات الزبون بعد المفاتيح الأربعة - التصميم المميز -->
                <div class="customer-card">
                    <!-- شريط أعلى مزخرف -->
                    <div style="position: absolute; top: 0; right: 0; left: 0; height: 4px; background: linear-gradient(90deg, #1a73e8, #34a853, #fbbc05);"></div>
                    
                    <!-- رأس القسم -->
                    <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid #f0f7ff;">
                        <div style="background: linear-gradient(135deg, #1a73e8, #4285f4); width: 50px; height: 50px; border-radius: 12px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 8px rgba(26, 115, 232, 0.2);">
                            <i class="fas fa-user-tie" style="color: white; font-size: 22px;"></i>
                        </div>
                        <div>
                            <h3 style="margin: 0; font-size: 18px; color: #1a73e8; font-weight: 700; text-shadow: 0 1px 2px rgba(0,0,0,0.05);">معلومات الزبون</h3>
                            <p style="margin: 4px 0 0 0; color: #5f6368; font-size: 12px; font-weight: 500;">بيانات العملية الحالية</p>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">
                        <!-- الزبون -->
                        <div style="background: #f8f9ff; border-radius: 12px; padding: 12px; border: 1px solid #e8f0fe; transition: transform 0.3s ease;">
                            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                                <div style="background: #e6f4ea; width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-user-check" style="color: #0d652d; font-size: 14px;"></i>
                                </div>
                                <div style="font-weight: 600; color: #5f6368; font-size: 11px; text-transform: uppercase;">الزبون</div>
                            </div>
                            <div style="font-size: 14px; color: #202124; font-weight: 600; padding-right: 5px; display: flex; align-items: center; gap: 5px;">
                        ...
                        <div style="background: #f8f9ff; border-radius: 12px; padding: 12px; border: 1px solid #e8f0fe; transition: transform 0.3s ease;">
                            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                                <div style="background: #fef7e0; width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-id-card" style="color: #fbbc05; font-size: 14px;"></i>
                                </div>
                                <div style="font-weight: 600; color: #5f6368; font-size: 11px; text-transform: uppercase;">رقم الزبون</div>
                            </div>
                            <div style="font-size: 14px; color: #202124; font-weight: 600; padding-right: 5px; display: flex; align-items: center; gap: 5px;">
                                <i class="fas fa-hashtag" style="color: #1a73e8; font-size: 12px;"></i>
                                <?= $walkin_debtor_no ?>
                            </div>
                        </div>
                        ...
                        <div style="background: #f8f9ff; border-radius: 12px; padding: 12px; border: 1px solid #e8f0fe; transition: transform 0.3s ease;">
                            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                                <div style="background: #e8f0fe; width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-warehouse" style="color: #1a73e8; font-size: 14px;"></i>
                                </div>
                                <div style="font-weight: 600; color: #5f6368; font-size: 11px; text-transform: uppercase;">المخزن</div>
                            </div>
                            <div style="font-size: 14px; color: #202124; font-weight: 500; line-height: 1.4; padding-right: 5px;">
                                موقع الشركة الرئيسي (DEF)
                            </div>
                        </div>
                        ...
                        <div style="background: #f8f9ff; border-radius: 12px; padding: 12px; border: 1px solid #e8f0fe; transition: transform 0.3s ease;">
                            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                                <div style="background: #fce8e6; width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-bullseye" style="color: #ea4335; font-size: 14px;"></i>
                                </div>
                                <div style="font-weight: 600; color: #5f6368; font-size: 11px; text-transform: uppercase;">مركز التكلفة</div>
                            </div>
                            <div style="font-size: 14px; color: #202124; font-weight: 500; line-height: 1.4; padding-right: 5px;">
                                مشروع جوالات 1 (1)
                            </div>
                        ...
                        <!-- نوع المبيعات -->
                        <div style="background: #f8f9ff; border-radius: 12px; padding: 12px; border: 1px solid #e8f0fe; transition: transform 0.3s ease; grid-column: span 2;">
                            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                                <div style="background: #f3e8ff; width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-shopping-cart" style="color: #9c27b0; font-size: 14px;"></i>
                                </div>
                                <div style="font-weight: 600; color: #5f6368; font-size: 11px; text-transform: uppercase;">نوع المبيعات</div>
                            </div>
                            <div style="display: flex; align-items: center; justify-content: space-between;">
                                <div style="font-size: 14px; color: #202124; font-weight: 500;">
                                    Retail
                                </div>
                                <div style="background: #e6f4ea; color: #0d652d; font-size: 11px; font-weight: 600; padding: 4px 8px; border-radius: 6px; border: 1px solid #34a853;">
                                    <i class="fas fa-check-circle"></i> الضريبة مضمنة
                                </div>
                            </div>
                        </div>
                        
                        <!-- نوع الضريبة -->
                        <div style="background: #f8f9ff; border-radius: 12px; padding: 12px; border: 1px solid #e8f0fe; transition: transform 0.3s ease; grid-column: span 2; background: linear-gradient(135deg, #f8f9ff 0%, #f0f7ff 100%);">
                            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                                <div style="background: linear-gradient(135deg, #0d652d, #34a853); width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; box-shadow: 0 3px 6px rgba(52, 168, 83, 0.2);">
                                    <i class="fas fa-percentage" style="color: white; font-size: 14px;"></i>
                                </div>
                                <div style="font-weight: 600; color: #5f6368; font-size: 11px; text-transform: uppercase;">نوع الضريبة</div>
                            </div>
                            <div style="display: flex; align-items: center; justify-content: space-between;">
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <div style="font-size: 15px; color: #202124; font-weight: 600;">
                                        Tax
                                    </div>
                                    <div style="background: #1a73e8; color: white; font-size: 12px; font-weight: 600; padding: 4px 8px; border-radius: 6px; min-width: 50px; text-align: center;">
                                        16%
                                    </div>
                                </div>
                                <div style="display: flex; align-items: center; gap: 5px; color: #5f6368; font-size: 11px;">
                                    <i class="fas fa-info-circle" style="color: #1a73e8;"></i>
                                    <span>نسبة الضريبة الحالية</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- تفاصيل إضافية -->
                    <div style="margin-top: 15px; padding: 12px; background: #f0f7ff; border-radius: 10px; border: 1px solid #cce0ff; display: flex; align-items: center; justify-content: space-between;">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-clock" style="color: #1a73e8;"></i>
                            <span style="font-size: 12px; color: #5f6368;">وقت النظام:</span>
                        </div>
                        <div id="currentTime" style="font-weight: 600; color: #1a73e8; font-size: 13px;"></div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        updateCurrentTime(); // التشغيل الأولي
