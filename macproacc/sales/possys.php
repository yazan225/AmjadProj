...
// ==============================================
// الحصول على جميع العملاء - جديد
// ==============================================
$customers = [];
$sql_customers = "SELECT debtor_no, name, debtor_ref, address FROM {$prefix}debtors_master 
                  WHERE inactive = 0 
                  ORDER BY name ASC";
...
// إذا لم توجد عملاء، ننشئ عميل افتراضي
if (count($customers) == 0) {
    // إنشاء عميل افتراضي
    $sql_insert_default_customer = "INSERT INTO {$prefix}debtors_master 
        (debtor_ref, name, address, tax_id, credit_status, payment_terms, discount, debtor_no) 
        VALUES ('Walk-In', 'زبون نقدي', 'عنوان افتراضي', '', 1, 4, 0, 1)";
    
    $conn->query($sql_insert_default_customer);
...
// البحث عن العميل Walk-In الافتراضي
$default_customer_id = null;
foreach ($customers as $customer) {
    if ($customer['debtor_ref'] == 'Walk-In' || stripos($customer['name'], 'walk-in') !== false) {
        $default_customer_id = $customer['debtor_no'];
        break;
    }
}

// إذا لم يتم العثور على Walk-In، نستخدم العميل الأول
if (!$default_customer_id && count($customers) > 0) {
    $default_customer_id = $customers[0]['debtor_no'];
}

// ==============================================
// تعيين القيم الافتراضية من الجلسة
// ==============================================
...
if (!isset($_SESSION['selected_customer']) && $default_customer_id) {
    $_SESSION['selected_customer'] = $default_customer_id;
}
...
// ==============================================
// التعامل مع تغيير العميل فقط
// ==============================================
if (isset($_POST['change_customer_only'])) {
    if (isset($_POST['customer'])) {
        $_SESSION['selected_customer'] = $_POST['customer'];
    }
    header("Location: " . $_SERVER['PHP_SELF']);
...
$default_customer = $_SESSION['selected_customer'] ?? $default_customer_id;
...
foreach ($customers as $customer) {
    if ($customer['debtor_no'] == $default_customer) {
        $selected_customer_data = $customer;
        break;
    }
}

// إذا لم يتم العثور على العميل المحدد، نستخدم الافتراضي
if (!$selected_customer_data && count($customers) > 0) {
    $selected_customer_data = $customers[0];
    $default_customer = $selected_customer_data['debtor_no'];
    $_SESSION['selected_customer'] = $default_customer;
}
...
// الحصول على زبون Walk-In للاستخدام في الفواتير القديمة
$walkin_debtor_no = $default_customer;

// الحصول على كود الفرع للعميل المحدد
$branch_code = "1";
...
if ($selected_customer_data) {
    ...
    $stmt_branch->bind_param("i", $selected_customer_data['debtor_no']);
    $stmt_branch->execute();
    ...
    if ($row_branch = $result_branch->fetch_assoc()) {
        ...
        $stmt_insert->bind_param("i", $selected_customer_data['debtor_no']);
...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    try {
        ...
        $debtor_data = [
            ...
            'type' => $type,
            'version' => 1,
            'debtor_no' => $selected_customer_data['debtor_no'], // استخدام العميل المحدد
        ...
        // الحصول على حساب المبيعات من sales_types إذا وجد - الطريقة الصحيحة
        if ($selected_sales_type_data) {
            // أولاً، نتحقق من الحقول الموجودة في جدول sales_types
            $sql_check_sales_fields = "DESCRIBE {$prefix}sales_types";
            $result_sales_fields = $conn->query($sql_check_sales_fields);
            $sales_types_fields = [];
            while ($field = $result_sales_fields->fetch_assoc()) {
                $sales_types_fields[] = $field['Field'];
            }
            
            // التحقق من الحقول المحتملة لحساب المبيعات
            $possible_sales_account_fields = ['sales_account', 'account', 'gl_account', 'gl_sales_account'];
            $found_sales_account_field = null;
            
            foreach ($possible_sales_account_fields as $field) {
                if (in_array($field, $sales_types_fields)) {
                    $found_sales_account_field = $field;
                    break;
                }
            }
            
            if ($found_sales_account_field) {
                $sql_get_account = "SELECT {$found_sales_account_field} as sales_account 
        ...
        foreach ($_SESSION['cart'] as $item) {
            ...
            $sql_check_stock_fields = "DESCRIBE {$prefix}stock_master";
            ...
            if (in_array('person_id', $gl_trans_fields)) {
                $gl_sales_data['person_id'] = $selected_customer_data['debtor_no'];
            }
        ...
        if (in_array('person_id', $gl_trans_fields)) {
            $gl_receivable_data['person_id'] = $selected_customer_data['debtor_no'];
        }
        if (in_array('dimension2_id', $gl_trans_fields)) {
        ...
        $_SESSION['last_invoice'] = [
            ...
            'customer_name' => $selected_customer_data ? $selected_customer_data['name'] : 'غير محدد',
            'customer_id' => $selected_customer_data ? $selected_customer_data['debtor_no'] : 0,
            'customer_ref' => $selected_customer_data ? $selected_customer_data['debtor_ref'] : '',
...
$_SESSION['success_message'] = "✅ تم حفظ الفاتورة بنجاح! - رقم الفاتورة: {$trans_no} - الإجمالي: " . number_format($total_for_invoice, 2) . " ₪ - التاريخ: " . date('Y-m-d H:i:s');
...
<head>
    ...
    <style>
        * {
        ...
        /* زر سريع لتغيير الزبون في المعلومات */
        .quick-change-customer {
            background: #4285f4;
            color: white;
            border: none;
            border-radius: 4px;
        ...
        #customerModal .customer-item:last-child {
            border-bottom: none;
        }
        
        #customerModal .customer-item-id {
            font-weight: bold;
            color: var(--primary-dark);
            min-width: 40px;
            text-align: center;
        ...
        #customerModal .customer-item-name {
            flex-grow: 1;
            text-align: right;
            margin: 0 10px;
            font-weight: 600;
        ...
        #customerModal .customer-item-ref {
            color: var(--text-light);
            font-size: 12px;
            min-width: 80px;
            text-align: left;
...
<body>
        ...
        <div class="content">
            <div class="panel right-panel">
                ...
                <form method="post" class="settings-grid-compact">
                    ...
                    <!-- السطر الثالث: اختيار العميل -->
                    <div class="setting-row-compact">
                        <label for="customer">الزبون:</label>
                        <select name="customer" id="customer" onchange="quickChangeCustomer()">
                            <?php foreach ($customers as $customer): ?>
                                <option value="<?= $customer['debtor_no'] ?>" <?= $customer['debtor_no'] == $default_customer ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($customer['name']) ?> (<?= htmlspecialchars($customer['debtor_ref']) ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="update-btn-compact">
                        <button type="submit" name="change_location" class="btn btn-primary" style="width: 100%;">
                            <i class="fas fa-sync-alt"></i> تحديث جميع الإعدادات
                        </button>
                    </div>
                </form>
                
                <!-- زر سريع لتغيير الزبون فقط -->
                <div style="margin-bottom: 10px;">
                    <form method="post" style="display: none;" id="quickCustomerForm">
                        <input type="hidden" name="customer" id="quickCustomerInput">
                        <input type="hidden" name="change_customer_only" value="1">
                    </form>
                </div>
                ...
                <div class="customer-compact-card">
                    ...
                    <div class="customer-compact-header">
                        ...
                        <button class="quick-change-customer" onclick="openCustomerModal()" title="تغيير الزبون">
                    ...
                    <div class="customer-compact-row">
                        <div class="customer-compact-item">
                            ...
                            <span class="customer-compact-value" title="<?= htmlspecialchars($selected_customer_data['name'] ?? 'غير محدد') ?>">
                                <?= htmlspecialchars(substr($selected_customer_data['name'] ?? 'غير محدد', 0, 12)) . (strlen($selected_customer_data['name'] ?? '') > 12 ? '...' : '') ?>
                                <?php if($selected_customer_data && $selected_customer_data['debtor_no'] == $default_customer): ?>
                                <span class="selected-customer-badge">محدد</span>
                                <?php endif; ?>
                            </span>
                        </div>
                        ...
                        <div class="customer-compact-item">
                            ...
                            <span class="customer-compact-value" title="<?= $selected_location_name ?>">
                                <?= htmlspecialchars(substr($selected_location_name, 0, 10)) . (strlen($selected_location_name) > 10 ? '...' : '') ?>
                            </span>
                        </div>
            ...
            <div id="printContent" style="background: white; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
                <div class="print-header">
                 ...
                 <div class="print-details">
                    ...
                    <div>
                        <div><strong>الزبون:</strong> <span id="printCustomer"><?= htmlspecialchars($selected_customer_data['name'] ?? 'غير محدد') ?></span></div>
                        <div><strong>رقم الزبون:</strong> <span id="printCustomerId"><?= $selected_customer_data['debtor_no'] ?? 0 ?></span></div>
    ...
    <div id="customerModal" class="modal">
        <div class="modal-content modal-large">
            <div class="modal-header">
                <h2 class="modal-title"><i class="fas fa-users"></i> اختيار الزبون</h2>
                <span class="close-btn" onclick="closeCustomerModal()">&times;</span>
            </div>
            <div class="search-container">
                <i class="fas fa-search search-icon"></i>
                <input type="text" id="customerSearchInput" class="payment-input" placeholder="ابحث عن زبون بالاسم أو الكود..." oninput="filterCustomers()" style="padding-right: 40px;">
            </div>
            <div class="customer-list" id="customersContainer">
                <div style="text-align: center; padding: 20px;">
                    <div class="loading" style="margin: 0 auto;"></div>
                    <p>جاري تحميل قائمة الزبائن...</p>
                </div>
            </div>
            <div class="action-buttons" style="margin-top: 20px;">
                <button class="btn btn-danger" onclick="closeCustomerModal()">
                    <i class="fas fa-times"></i> إغلاق
                </button>
    ...
    <script>
        ...
        let currentCustomer = <?= json_encode($selected_customer_data) ?>;
...
الزبون: ${lastInvoice.customer_name}
...
نوع الضريبة: ${lastInvoice.tax_name}`;
        ...
        function printInvoice(type) {
            ...
            let customerName = '<?= htmlspecialchars($selected_customer_data['name'] ?? 'غير محدد') ?>';
            let customerId = '<?= $selected_customer_data['debtor_no'] ?? 0 ?>';
            ...
            if (cartItems.length > 0) {
                ...
                customerName = lastInvoice.customer_name || customerName;
                customerId = lastInvoice.customer_id || customerId;
            } else {
            ...
            if (type === 'a4') {
                ...
                document.getElementById('printCustomerId').textContent = customerId;
                ...
                receiptContent.innerHTML += `الزبون: ${customerName}\n`;
                receiptContent.innerHTML += `رقم الزبون: ${customerId}\n`;
        ...
        function renderCustomers(customers) {
            ...
            customers.forEach(customer => {
                ...
                const isSelected = customer.debtor_no == currentCustomer.debtor_no;
                
                customerDiv.innerHTML = `
                    <span class="customer-item-id">${customer.debtor_no}</span>
                    <span class="customer-item-name">
                        ${customer.name}
                        ${isSelected ? '<span class="selected-customer-badge">محدد</span>' : ''}
                    </span>
                    <span class="customer-item-ref">${customer.debtor_ref}</span>
                `;
                
                customerDiv.onclick = () => selectCustomer(customer.debtor_no);
        ...
        function filterCustomers() {
            ...
            const filteredCustomers = allCustomers.filter(customer => 
                ...
                customer.debtor_ref.toLowerCase().includes(searchTerm) ||
                customer.debtor_no.toString().includes(searchTerm)
            );
            renderCustomers(filteredCustomers);
        }
        
        function selectCustomer(customerId) {
            document.getElementById('quickCustomerInput').value = customerId;
            document.getElementById('quickCustomerForm').submit();
        }
        ...
        function quickChangeCustomer() {
            const customerSelect = document.getElementById('customer');
            const selectedCustomerId = customerSelect.value;
            document.getElementById('quickCustomerInput').value = selectedCustomerId;
            document.getElementById('quickCustomerForm').submit();
        }
        
        function addNewCustomer() {
            alert('سيتم إضافة هذه الميزة في نسخة قادمة.');
        }
    </script>
</body>
</html>