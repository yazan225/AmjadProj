...
if (count($tax_types) == 0) {
    $tax_types[] = ['id' => 1, 'name' => 'ضريبة القيمة المضافة', 'rate' => 17, 'sales_gl_code' => '21040001', 'purchasing_gl_code' => ''];
}
...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    try {
        ...
        if ($selected_tax_data) {
            $tax_rate = floatval($selected_tax_data['rate']) / 100;
        ...
        // قيد الضريبة باستخدام كود الحساب من جدول الضرائب
        ...
        // إتمام المعاملة
        $conn->commit();
        $success = "تم حفظ الفاتورة بنجاح! رقم: $trans_no (طلب المبيعات: $order_no) - العملة: $main_currency - الموقع: $default_location - مركز التكلفة: $dimension_id - الضريبة: " . ($selected_tax_data ? $selected_tax_data['name'] . ' (' . $selected_tax_data['rate'] . '%)' : 'غير محدد');
