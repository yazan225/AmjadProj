...
if (isset($_GET['action']) && $_GET['action'] == 'excel') {
    ...
    if (!empty($from) && !empty($to)) {
        ...
        echo "<h1 style='text-align: center;'>تقرير الأرباح والخسائر</h1>";
        ...
        $revenue_sql = "SELECT SUM(amount) as total FROM " . TB_PREF . "gl_trans 
                        WHERE tran_date BETWEEN '$from_sql' AND '$to_sql' AND account LIKE '41%'";
        ...
        $cogs_sql = "SELECT SUM(amount) as total FROM " . TB_PREF . "gl_trans 
                     WHERE tran_date BETWEEN '$from_sql' AND '$to_sql' AND account LIKE '42%'";
        ...
        $sales_exp_sql = "SELECT SUM(amount) as total FROM " . TB_PREF . "gl_trans 
                          WHERE tran_date BETWEEN '$from_sql' AND '$to_sql' AND account LIKE '43%'";
        ...
        $admin_exp_sql = "SELECT SUM(amount) as total FROM " . TB_PREF . "gl_trans 
                          WHERE tran_date BETWEEN '$from_sql' AND '$to_sql' AND account LIKE '44%'";
        ...
        $other_exp_sql = "SELECT SUM(amount) as total FROM " . TB_PREF . "gl_trans 
                          WHERE tran_date BETWEEN '$from_sql' AND '$to_sql' AND account LIKE '52%'";
        ...
        $other_rev_sql = "SELECT SUM(amount) as total FROM " . TB_PREF . "gl_trans 
                          WHERE tran_date BETWEEN '$from_sql' AND '$to_sql' AND account LIKE '51%'";
        ...
        echo "<td colspan='2' style='text-align: center; font-size: 1.2em;'>";
...
if (isset($_POST['Generate'])) {
    ...
    if (!empty($from) && !empty($to)) {
        ...
        echo "<a href='testdata5.php' style='padding: 12px 25px; background: #3498db; color: white; text-decoration: none; border-radius: 6px; margin: 5px; display: inline-block;'>🔄 تقرير جديد</a>";
        echo "<a href='testdata5.php?action=print&from=" . urlencode($from) . "&to=" . urlencode($to) . "' target='_blank' style='padding: 12px 25px; background: #27ae60; color: white; text-decoration: none; border-radius: 6px; margin: 5px; display: inline-block;'>🖨️ طباعة التقرير</a>";
        echo "<a href='testdata5.php?action=excel&from=" . urlencode($from) . "&to=" . urlencode($to) . "' style='padding: 12px 25px; background: #f39c12; color: white; text-decoration: none; border-radius: 6px; margin: 5px; display: inline-block;'>📊 تصدير Excel</a>";
        echo "</div>";
        ...
        echo "<h1 style='color: #2c3e50; margin: 0; font-size: 28px;'>تقرير الأرباح والخسائر</h1>";
...
echo "<p style='color: #7f8c8d; margin-bottom: 30px;'>اختر الفترة الزمنية لتوليد التقرير</p>";
