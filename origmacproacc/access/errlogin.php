...
$available_companies = array(
    ...
    'alfoula' => array(
        'id' => 4,
        'name' => 'lolo',
        'display_name' => 'الشركة الرابعة'
    )

);

// Validate company name from URL - REQUIRED for login
if (empty($company_name_from_url)) {
    // No company specified - show error and prevent login
    $login_blocked = true;
    $fixed_company_name = "غير محدد";
    $company_id = -1;
    $demo_text = '<span class="redfg">' . _('لطفا قم بادخال الرابط المخصص لشركتكم.....') . '</span>';
} elseif (!array_key_exists($company_name_from_url, $available_companies)) {
    // Invalid company specified - show error and prevent login
    $login_blocked = true;
    $fixed_company_name = "غير صالح";
    $company_id = -1;
    $demo_text = '<span class="redfg">' . _('اسم الشركة غير صحيح. يرجى استخدام رابط الدخول المخصص لشركتكم.') . '</span>';
} else {
    // Valid company - proceed normally
...
// Debug output
error_log("Login attempt - Company: " . $company_name_from_url . ", ID: " . $company_id . ", Blocked: " . ($login_blocked ? 'Yes' : 'No'));
...
function defaultCompany()
{
    ...
    if ($SysPrefs->allow_demo_mode == true && !isset($login_blocked))
    {
        $demo_text = _("الدخول كمسخدم: demouser وكلمة المرور: password");
    }
    else if (!isset($login_blocked))
    {
        $demo_text = _("يرجى تسجيل الدخول هنا");
        if (@$SysPrefs->allow_password_reset) {
            $demo_text .= " "._("أو")." <a href='$path_to_root/index.php?reset=1'>"._("طلب كلمة مرور جديدة")."</a>";
        }
    }

    if (check_faillog() && !isset($login_blocked))
    {
        $blocked = true;
    ...
    } elseif ($_SESSION["wa_current_user"]->login_attempt > 1 && !isset($login_blocked)) {
    ...
    echo "        }\n";
    echo "\n";
    echo "        .company-badge.error {\n";
    echo "            background: linear-gradient(135deg, var(--error), #c1121f);\n";
    echo "        }\n";
    echo "\n";
    echo "        .form-group {\n";
    echo "            margin-bottom: 15px;\n";
    ...
    echo "        }\n";
    echo "\n";
    echo "        .btn-login:disabled {\n";
    echo "            background: linear-gradient(135deg, #6c757d, #495057);\n";
    echo "            cursor: not-allowed;\n";
    echo "            transform: none;\n";
    echo "            box-shadow: none;\n";
    echo "        }\n";
    echo "\n";
    echo "        .btn-login:disabled::before {\n";
    echo "            display: none;\n";
    echo "        }\n";
    echo "\n";
    ...
    echo "                    <h1>النظام المحاسبي الموحد</h1>\n";
    if ($company_id >= 0) {
        echo "                    <div class='company-badge'>شاشة الدخول لشركة ".$fixed_company_name."</div>\n";
    } else {
        echo "                    <div class='company-badge error'>شركة غير محددة</div>\n";
    }
    echo "                </div>\n";
    echo "            </div>\n";
    ...
    if ($company_id >= 0) {
        echo "                <p>لطفاً قم بتسجيل الدخول للبيانات الخاص بكم <strong>".$fixed_company_name."</strong></p>\n";
    } else {
        echo "                <p style='color: var(--error); font-weight: 500;'>يجب استخدام رابط الدخول الصحيح الخاص بشركتكم</p>\n";
    }
    echo "            </div>\n";
    echo "\n";
    ...
    } elseif (check_faillog() || ($_SESSION["wa_current_user"]->login_attempt > 1 && !$login_timeout)) {
        echo '<div class="alert alert-error">' . $demo_text . '</div>';
    } elseif ($SysPrefs->allow_demo_mode == true && !isset($login_blocked)) {
    ...
    if (!$allow) {
        echo '<div class="alert alert-error">' . _("الوصول عبر HTTP غير مسموح به على هذا الموقع. هذا غير آمن. إذا كنت ترغب حقًا في الوصول إلى هذا الموقع غير الآمن ، فاضبط SECURE_ONLY على false في ملف /includes/session.inc.") . '</div>';
    }
    ...
    if ($login_timeout) {
        ...
        echo "                <input type='hidden' name='company' value='".$company_name_from_url."'>\n";
        ...
        if ($allow) {
            ...
            echo "                    <div class='input-with-icon'>\n";
            echo "                        <i class='fas fa-user'></i>\n";
            echo "                        <input type='text' id='user_name_entry_field' name='user_name_entry_field' value='$value' class='form-control' placeholder='أدخل اسم المستخدم' required";
            echo (isset($login_blocked) && $login_blocked) ? " disabled" : "";
            echo ">\n";
            echo "                    </div>\n";
            echo "                </div>\n";
            ...
            if (isset($blocked) || (isset($login_blocked) && $login_blocked)) {
                echo " disabled";
            }
            echo ">\n";
            if ($company_id >= 0) {
                ...
                echo "                    <i class='fas fa-ban'></i> الدخول غير متاح\n";
            }
            echo "                </button>\n";
            
            // Hidden fields for timeout data
            foreach($_SESSION['timeout']['post'] as $p => $val) {
                if (!in_array($p, array('ui_mode', 'user_name_entry_field', 'password', 'SubmitUser', 'company_login_name', 'company'))) {
                    if (!is_array($val))
    ...
    if (@$SysPrefs->allow_password_reset && !(isset($login_blocked) && $login_blocked)) {
    ...
    echo "                    <li><i class='fas fa-chart-line'></i> إدارة المخزون/الزبائن/الموردين</li>\n";
    echo "                    <li><i class='fas fa-boxes'></i> إدارة المبيعات والمشتريات ونقاط البيع</li>\n";
    echo "                    <li><i class='fas fa-handshake'></i> متعدد العملات/الشركات/المستودعات</li>\n";
    echo "                    <li><i class='fas fa-globe'></i> يشمل الاستاذ العام وادوات التصنيع ومراكز التكلفة</li>\n";
    echo "                    <li><i class='fas fa-file-invoice'></i>يشمل تحليلات مالية وجداول احصائية</li>\n";
    echo "                    <li><i class='fas fa-file-invoice'></i>لوحة تحكم شاملة لكل شركة</li>\n";
    echo "                </ul>\n";
    echo "                \n";
    ...
    echo "            }\n";
    echo "            \n";
    echo "            // Set focus to username field only if form is not blocked\n";
    echo "            if (document.forms.length && document.forms[0].user_name_entry_field && !document.forms[0].user_name_entry_field.disabled) {\n";
