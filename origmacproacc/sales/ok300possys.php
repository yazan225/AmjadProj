...
// 🔥 الإصدار المحسن - استبدل دالة getStockItems بأكملها بهذا الكود
function getStockItems($conn, $prefix, $location) {
    ...
    $sql_all_items = "
        SELECT 
            ...
            -- الأولوية لسعر البيع من جدول الأسعار
            COALESCE(
...
if (isset($_POST['add_item'])) {
    ...
    if ($found_index >= 0) {
        ...
        // 🔥 التعديل هنا: جلب material_cost بدلاً من purchase_cost
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            if ($price_row = $res_price->fetch_assoc()) {
                ...
                $unit_price = $row['material_cost']; // 🔥 استخدام material_cost بدلاً من purchase_cost
            ...
            $cart[] = [
                ...
                'currency' => $currency,
                'standard_cost' => floatval($row['material_cost']), // 🔥 استخدام material_cost هنا
                'qty' => $qty,
                'discount' => 0,
...
// 🔥 الإصلاح - إزالة الـ $ الإضافية
if (isset($_GET['remove'])) {
    $remove_id = $_GET['remove'];
    foreach ($_SESSION['cart'] as $k => $item) { // ✅ تم التصحيح
    ...
    throw new Exception("فشل في إنشاء طلب المبيعات: " . $stmt_order->error);
        ...
        foreach ($_SESSION['cart'] as $item) {
            ...
            $stmt_stock_move->bind_param("iisssdsdd", 
                ...
                $reference, 
                $qty, 
                $item['standard_cost']  // 🔥 هنا يتم تمرير material_cost كـ standard_cost
            );
