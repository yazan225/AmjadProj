...
if (isset($_POST['update_cart'])) {
    ...
    // الخصم الكلي يبقى كنسبة مئوية
    $invoice_disc = floatval($_POST['invoice_discount'] ?? 0);
    if ($invoice_disc >= 0 && $invoice_disc <= 100) {
        $_SESSION['invoice_discount'] = $invoice_disc;
    }
    header("Location: " . $_SERVER['PHP_SELF']);
    exit;
}
    ...
    if ($found_index >= 0) {
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $cart[] = [
                ...
                'qty' => $qty,
                'discount' => 0, // تخزين كنسبة مئوية (0%)
...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    ...
    $invoice_discount_percentage = $_SESSION['invoice_discount']; // هذه نسبة مئوية (مثلاً 7)
    $invoice_discount_decimal = $invoice_discount_percentage / 100; // التحويل لكسر عشري للحساب (0.07)
    ...
    $total_cost = 0;
    foreach ($_SESSION['cart'] as $item) {
        $item_discount_decimal = $item['discount'] / 100; // تحويل خصم الصنف لكسر عشري للحساب
        $line_total = $item['unit_price'] * $item['qty'] * (1 - $item_discount_decimal);
        $sub_total += $line_total;
        $total_cost += $item['standard_cost'] * $item['qty'];
    }
    $total = $sub_total * (1 - $invoice_discount_decimal);
    ...
    try {
        ...
        $stmt = $conn->prepare($sql);
        $due_date = $tran_date;
        $discount_amount = $sub_total * $invoice_discount_decimal;
        ...
        // إدخال الأصناف - تحويل الخصم لكسر عشري لقاعدة البيانات
        $line_no = 1;
        foreach ($_SESSION['cart'] as $item) {
            $item_discount_decimal = $item['discount'] / 100; // تحويل لكسر عشري للحساب
            $line_total = $item['unit_price'] * $item['qty'] * (1 - $item_discount_decimal);
            ...
            // 🔥 التحويل لكسر عشري للتخزين في قاعدة البيانات (7% تصبح 0.07)
            ...
            $stmt_line->bind_param("iissddidii",
                ...
                $unit_tax, 
                $item['qty'], 
                $discount_percent,  // تخزين كـ 0.07 في قاعدة البيانات
                $item['standard_cost'], 
                $item['qty']
            );
        ...
        $_SESSION['last_invoice'] = [
            ...
            'total' => $total,
            'items' => $_SESSION['cart'],
            'discount' => $invoice_discount_percentage // تخزين كنسبة مئوية
        ];
        $_SESSION['cart'] = [];
        $_SESSION['invoice_discount'] = 0;
                    ...
                    <div class="cart-header">
                        ...
                        <div class="total-amount">
                            <?php 
                                ...
                                // تحويل الخصم الكلي لكسر عشري للحساب
                                $invoice_discount_decimal = $_SESSION['invoice_discount'] / 100;
                                $discounted_total = $sub_total * (1 - $invoice_discount_decimal);
                        ...
                        <form method="post">
                            ...
                            <div class="summary">
                                ...
                                <div class="summary-row">
                                    ...
                                    <!-- العرض كنسبة مئوية -->
                                    <span><?= ($_SESSION['invoice_discount'] ?? 0) ?>%</span>
                                </div>
                                ...
                                <div class="discount-input">
                                    ...
                                    <input type="number" name="invoice_discount" step="0.1" 
                                           value="<?= ($_SESSION['invoice_discount'] ?? 0) ?>" min="0" max="100">
                                    <span>%</span>
                                </div>
    ...
    <script>
        ...
        function openPaymentModal() {
            ...
            if (cartItems.length > 0) {
                ...
                const invoiceDiscountDecimal = invoiceDiscount / 100;
                total = subTotal * (1 - invoiceDiscountDecimal);
                currentInvoiceNo = '';
        ...
        function printInvoice(type) {
            ...
            if (cartItems.length > 0) {
                ...
                const invoiceDiscountDecimal = invoiceDiscount / 100;
                discountAmount = subTotal * invoiceDiscountDecimal;
                ...
                const invoiceDiscountDecimal = lastInvoice.discount / 100;
                discountAmount = subTotal * invoiceDiscountDecimal;
