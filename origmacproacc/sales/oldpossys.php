...
function getStockItems($conn, $prefix, $location) {
    ...
    $sql_all_items = "
        ...
        GROUP BY 
            sm.stock_id, sm.description, sm.purchase_cost
        ORDER BY 
            sm.description ASC";
    ...
    if ($result_all_items) {
        while ($row_item = $result_all_items->fetch_assoc()) {
            // Try to get price in ILS first, if not found try USD, otherwise use purchase cost
            $sql_price_modal = "SELECT price, curr_abrev FROM {$prefix}prices WHERE stock_id = ? AND (curr_abrev = 'ILS' OR curr_abrev = 'USD') ORDER BY curr_abrev = 'ILS' 

DESC LIMIT 1";
            $stmt_price_modal = $conn->prepare($sql_price_modal);
            $stmt_price_modal->bind_param("s", $row_item['stock_id']);
            $stmt_price_modal->execute();
            $res_price_modal = $stmt_price_modal->get_result();
            
            if ($price_row = $res_price_modal->fetch_assoc()) {
                $unit_price_modal = $price_row['price'];
                $currency = $price_row['curr_abrev'];
            } else {
                $unit_price_modal = $row_item['purchase_cost'];
                $currency = 'ILS'; // Default currency
            }
            $stmt_price_modal->close();

            $all_stock_items[] = [
                'stock_id' => $row_item['stock_id'],
                'description' => $row_item['description'],
                'unit_price' => floatval($unit_price_modal),
                'currency' => $currency,
                'purchase_cost' => floatval($row_item['purchase_cost']),
...
if (isset($_POST['add_item'])) {
    ...
    if ($found_index >= 0) {
        ...
        if ($row = $result->fetch_assoc()) {
            // Try to get price in ILS first, if not found try USD, otherwise use purchase cost
            $sql_price = "SELECT price, curr_abrev FROM {$prefix}prices WHERE stock_id = ? AND (curr_abrev = 'ILS' OR curr_abrev = 'USD') ORDER BY curr_abrev = 'ILS' DESC 
...
if (isset($_POST['save_invoice']) && count($_SESSION['cart']) > 0) {
    startNewInvoice(); // مسح الفواتير السابقة قبل حفظ الجديدة
    ...
    // استخدام رقم الفاتورة كمرجع (بدون بادئة POS-)
    $reference = $trans_no;
    ...
    $sub_total = 0;
    $total_cost = 0; // إجمالي التكلفة للمنتجات
    foreach ($_SESSION['cart'] as $item) {
        $line_total = $item['unit_price'] * $item['qty'] * (1 - $item['discount'] / 100);
        $sub_total += $line_total;
        $total_cost += $item['standard_cost'] * $item['qty']; // حساب التكلفة الإجمالية
    }
...
$stmt_order->bind_param("isssssissd", 
    ...
    $branch_code,        // s
    $order_ref,          // s
    $tran_date,          // s (ord_date)
    $ship_via,           // i
    $default_location,   // s
    $due_date,           // s (delivery_date)
    $payment_term,       // i
    $total               // d
);
...
if (!$stmt_order->execute()) {
    throw new Exception("فشل في إنشاء طلب المبيعات: " . $stmt_order->error);
        ...
        $stmt->bind_param("iiissssiisiddi", 
            ...
            $total, 
            $discount_amount,
            $order_no  // Link to the sales order
        );
        ...
        foreach ($_SESSION['cart'] as $item) {
            ...
            // Calculate tax for this item (assuming 17% tax rate)
            $tax_rate = 0.17;
            ...
            $qty = -abs($item['qty']); // Negative for sale
        ...
        // 1. الحصول على حسابات الشركة الافتراضية
       $sql_accounts = "SELECT sales_account, receivable_account, inventory_account, cogs_account 
        ...
        if ($result_accounts && $result_accounts->num_rows > 0) {
            ...
            // استخدام حسابات افتراضية إذا لم توجد في جدول الشركة
            $sales_account = 41010001;      // حساب المبيعات
            $receivable_account = 11011001; // حساب البنك
            $inventory_account = 1400;      // حساب المخزون
            $cogs_account = 5000;           // حساب تكلفة المبيعات
        }

        // حساب الضريبة (نفترض نسبة 17% للضريبة)
        $tax_rate = 0.17;
        $tax_amount = $total * $tax_rate;
        $net_amount = $total - $tax_amount;

        // 2. إدخال قيود المبيعات والذمم المدينة والضريبة
        ...
        // قيد المبيعات: زيادة المبيعات (دائن) - صافي المبلغ بدون ضريبة
        $sales_amount = -$net_amount; // سالب لأنها دائن
        ...
        // قيد الضريبة: زيادة ضريبة المبيعات (دائن)
        $tax_amount_negative = -$tax_amount; // سالب لأنها دائن
        ...
        // 3. إدخال قيود تكلفة المبيعات والمخزون
        if ($total_cost > 0) {
            // قيد تكلفة المبيعات: زيادة تكلفة المبيعات (مدين)
            ...
            // قيد المخزون: انخفاض المخزون (دائن)
            $inventory_amount = -$total_cost; // سالب لأنها دائن
...
if (isset($_POST['process_payment'])) {
    ...
    if ($change < 0) {
        ...
        // استخدام رقم الفاتورة كمرجع للدفعة (بدون بادئة PAY-)
                    ...
                    <div class="cart-header">
                        ...
                        <div class="total-amount">
                            <?php 
                                ...
                                $discounted_total = $sub_total * (1 - ($_SESSION['invoice_discount'] ?? 0) / 100);
    ...
    <script>
        ...
        function printInvoice(type) {
            ...
            if (cartItems.length > 0) {
                ...
                discountAmount = subTotal * (lastInvoice.discount / 100);
            ...
            if (type === 'a4') {
                ...
                document.getElementById('printDiscountPercent').textContent = invoiceDiscount;
                ...
                receiptContent.innerHTML += `الخصم الكلي (${invoiceDiscount}%): ${discountAmount.toFixed(2)} ₪\n`;
